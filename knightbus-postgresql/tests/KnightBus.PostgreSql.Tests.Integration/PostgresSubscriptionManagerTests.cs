...
public class PostgresSubscriptionManagerTests : QueueManagerTests<PostgresTestEvent>
{
    ...
    public override async Task Setup()
    {
        ...
        _postgresQueueClient = new PostgresSubscriptionClient<PostgresTestEvent>(PostgresSetup.DataSource, new MicrosoftJsonSerializer(), _eventSubscription);
        QueueManager = new PostgresSubscriptionManager(AutoMessageMapper.GetQueueName<PostgresTestEvent>(), _postgresManagementClient,
            new PostgresConfiguration { MessageSerializer = new MicrosoftJsonSerializer() });
    ...
    public override async Task<string> CreateQueue()
    {
        ...
        await QueueInitializer.InitSubscription(PostgresQueueName.Create(AutoMessageMapper.GetQueueName<PostgresTestEvent>()), PostgresQueueName.Create(subscription), PostgresSetup.DataSource);
        return subscription;
    }
    ...
    public override async Task<IMessageStateHandler<PostgresTestEvent>> GetMessageStateHandler(string queueName)
    {
