...
WHERE message_id = {message[0].Id}"
    ...
    [Test]
    public async Task GetDeadLetterMessageCountAsync_ShouldReturn1_WhenDeadletterInQueue()
    {
        await _postgresBus.SendAsync<TestCommand>(
            [new TestCommand { MessageBody = "dead letter" }],
            default
        ...
        var result = await _postgresQueueClient.GetDeadLetterMessageCountAsync(default);
        result.Should().Be(1);
    }

    [Test]
    public async Task GetDeadLetterMessageCountAsync_ShouldReturn0_WhenNoDeadletterInQueue()
    {
        await _postgresBus.SendAsync<TestCommand>(
            [new TestCommand { MessageBody = "hello, world!" }],
            default
