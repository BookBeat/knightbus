...
class ProcessedTracker()
{
    ...
    public static ConcurrentDictionary<string, int> processed { get; set; } = new ConcurrentDictionary<string, int>();
    ...
    public static int Processed(IEnumerable<string> messageStrings, int deliveriesPerMessage = 1)
    {
        ...
        {
            if (ProcessedTracker.processed.TryGetValue(id, out var value) && value >= deliveriesPerMessage)
            {
                processed++;
            }
        }
        ...
        int TimeOutSeconds = 10)
    {
        ...
        while (Processed(messageStrings, deliveriesPerMessage) < numMessages*deliveriesPerMessage
