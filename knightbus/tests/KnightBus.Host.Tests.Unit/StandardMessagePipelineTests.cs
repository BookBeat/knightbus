...
public class StandardMessagePipelineTests
{
    ...
    public void Setup()
    {
        ...
        _messageHandlerProvider
            .Setup(x =>
                x.GetInstance<IProcessMessage<TestCommandOne, Task>>(
                    typeof(MultipleCommandProcessor)
                )
            )
        ...
        _hostConfiguration
            .Setup(x => x.DependencyInjection)
            .Returns(_messageHandlerProvider.Object);
        ...
        var middlewares = new List<IMessageProcessorMiddleware> { new ThrottlingMiddleware(1) };
        var pipeline = new MiddlewarePipeline(
            middlewares,
            _pipelineInformation.Object,
            _logger.Object
        );
        _messageProcessor = pipeline.GetPipeline(
            new MessageProcessor(typeof(MultipleCommandProcessor))
        );
    }
