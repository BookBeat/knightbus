...
namespace KnightBus.Core.Tests.Unit
{
    ...
    public class SagaMiddlewareTests
    {
        ...
        public async Task Should_complete_message_if_saga_already_is_started()
        {
            ...
            sagaStore.Setup(x => x.Create<SagaData>(partitionKey, id, It.IsAny<SagaData>(), TimeSpan.Zero)).ThrowsAsync(new SagaAlreadyStartedException(partitionKey, id));
        ...
        public async Task Should_load_sagadata_and_call_next_when_success()
        {
            ...
            var id = "b";
            var sagaStore = new Mock<ISagaStore>();
            sagaStore.Setup(x => x.Create(partitionKey, id, It.IsAny<SagaData>(), TimeSpan.Zero)).ReturnsAsync(new SagaData{Data = "loaded"});
