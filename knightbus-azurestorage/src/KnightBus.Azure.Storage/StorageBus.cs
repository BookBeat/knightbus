...
public interface IStorageBus
{
    ...
    Task ScheduleAsync<T>(T command, TimeSpan delay, CancellationToken cancellationToken = default)
        where T : class, IStorageQueueCommand;
}
...
public class StorageBus : IStorageBus
{
    ...
    private readonly ConcurrentDictionary<Type, IStorageQueueClient> _queueClients =
        new ConcurrentDictionary<Type, IStorageQueueClient>();
    private readonly IEnumerable<IMessagePreProcessor> _messagePreProcessors;

    public StorageBus(
        IStorageBusConfiguration options,
    ...
    private IStorageQueueClient GetClient<T>()
        where T : class, ICommand
    {
        return _queueClients.GetOrAdd(
            ...
            type =>
            {
                ...
                return new StorageQueueClient(
                    _options,
                    serializer,
                    _messagePreProcessors,
