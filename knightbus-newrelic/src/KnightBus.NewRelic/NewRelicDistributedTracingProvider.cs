...
public class NewRelicDistributedTracingProvider : IDistributedTracingProvider
{
    ...
    {
        NewRelic.Api.Agent.NewRelic.GetAgent().CurrentTransaction.AcceptDistributedTraceHeaders(properties,
            (carrier, key) => new[] { carrier[key] }, TransportType.Queue);
    }

    public IDictionary<string, string> GetProperties()
    {
        var result = new Dictionary<string, string>();
        NewRelic.Api.Agent.NewRelic.GetAgent().CurrentTransaction.InsertDistributedTraceHeaders(result,
