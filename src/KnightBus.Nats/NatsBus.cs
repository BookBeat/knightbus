...
public interface INatsBus
{
    ...
    Task<TResponse> RequestAsync<T, TResponse>(
        INatsRequest request,
...
public class NatsBus : INatsBus
{
    ...
    public async Task<TResponse> RequestAsync<T, TResponse>(
        INatsRequest request,
        CancellationToken cancellationToken = default
    )
        where T : INatsRequest
    ...
    private void ThrowIfErrorResponse(Msg msg)
    {
        if (
            msg.Data.Length == 0
            && msg.HasHeaders
            && msg.Header[MsgConstants.HeaderName] == MsgConstants.Error
        )
        {
