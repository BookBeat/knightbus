...
public class ExtendMessageLockDurationMiddleware : IMessageProcessorMiddleware
{
        ...
        where T : class, IMessage
    {
        ...
        {
            if (
                messageStateHandler is IMessageLockHandler<T> lockHandler
                && pipelineInformation.ProcessingSettings is IExtendMessageLockTimeout extendLock
            )
            {
                var token = cts.Token;
#pragma warning disable 4014
                Task.Run(
                        ...
                        {
                            await RenewLock(
                                    extendLock.ExtensionInterval,
                                    extendLock.ExtensionDuration,
                                    token,
                                    lockHandler,
                                    pipelineInformation.HostConfiguration.Log
                                )
...
#pragma warning restore 4014
    ...
    private async Task RenewLock<T>(
        TimeSpan interval,
        TimeSpan duration,
        CancellationToken cancellationToken,
        IMessageLockHandler<T> lockHandler,
        ILogger log
    )
        where T : class, IMessage
    {
        while (!cancellationToken.IsCancellationRequested)
        {
            try
            {
                ...
                if (cancellationToken.IsCancellationRequested)
                    return;
                await lockHandler
                    .SetLockDuration(duration, cancellationToken)
                    .ConfigureAwait(false);
            }
            catch (Exception e)
            {
